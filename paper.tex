...
\begin{enumerate}
	\item \textbf{Zustände} beschreiben jeweils einen bestimmten Zustand in dem man sich befinden kann. Von jedem Zustand aus gibt es eine bestimmte Menge von ausführbaren Aktionen. Die Menge der Zustände bezeichnen wir als $S$.
	\item \textbf{Aktionen} beschreiben Dinge, die ausgeführt werden können. Das Hauptproblem beim Lösen von MDPs ist eine beste Aktion für jeden Zustand zu finden. Die Menge an Zuständen bezeichnen wir als $A$.
	\item Der \textbf{Übergang} $P(s, a, s')$ drückt die Wahrscheinlichkeit aus, dass man sich nach Wählen der Aktion $a$ in Zustand $s$ im Zustand $s'$ befindet. $P$ ist also die Abbildung
	\begin{equation}
		\begin{split}
		...
		\end{split}
	\end{equation}
	\item Die \textbf{Gütefunktion} $R$ drückt sowohl die Belohnung als auch die Kosten für bestimmte Übergänge aus. Kosten werden als negative Zahlen und Belohnungen als positive Zahlen dargestellt. $R$ ist also die Abbildung
	\begin{equation}

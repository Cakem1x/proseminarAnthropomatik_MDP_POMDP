...
Dazu betrachten wir zuerst einen beschränkten Planungshorizont. Eine optimale Strategie 
\begin{equation}
	\pi_1: s \rightarrow a
\end{equation}
...
Für den Planungshorizont $n=2$ können wir nun rekursiv auf die schon definierten Funktionen für den Planungshorizont $n=1$ aufbauen. Die optimale Strategie für $n=2$ wählt für jeden Zustand $s$ jene Aktion $a$, welche die Summe aus der sofortigen Belohnung $r(s,a)$ und der erwarteten Belohnung aus der Wertefunktion $V_1(s)$  maximiert
\begin{equation}
	\pi_2(s) = \underset{a}{argmax} \left[ r(s,a) + \int V_1(s')\ p(s, a, s')\ ds' \right]\ .
\end{equation}
Die entsprechende Wertefunktion lautet dann
\begin{equation}
	V_2(s) = \gamma\ \underset{a}{max} \left[ r(s,a) + \int V_1(s')\ p(s, a, s')\ ds' \right]\ .
\end{equation}

...
\begin{enumerate}
	...
	\item \textbf{Reduzierte Güte:} (englisch: discounted rewards)
		...
		\begin{equation}
			V = \sum\limits_{t=0}^n \gamma^t r(s_t, a_t) \leq \sum\limits_{t=0}^n \gamma^t r_{max} = \frac{r_{max}}{1-\gamma}  \ .
		\end{equation}
	\item
...
\begin{equation}
	\pi^* = \underset{\pi}{argmax}\ \E \left[ \sum\limits_{t=0}^{\infty} \gamma^t r(s_t, a_t) \ \vert\ \pi \right]\ .
\end{equation}
